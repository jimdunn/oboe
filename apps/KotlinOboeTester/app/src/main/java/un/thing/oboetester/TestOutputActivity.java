...
public final class TestOutputActivity extends TestOutputActivityBase {
    ...
    public void startTestUsingBundle() {
        try {
            ...
            IntentBasedTestSupport.configureOutputStreamFromBundle(getMBundleFromIntent(), requestedOutConfig);

            int signalType = IntentBasedTestSupport.getSignalTypeFromBundle(getMBundleFromIntent());
            mAudioOutTester.setSignalType(signalType);

            openAudio();
            startAudio();

            int durationSeconds = IntentBasedTestSupport.getDurationSeconds(getMBundleFromIntent());
