...
import un.thing.miditools.MidiOutputPortConnectionSelector;
import un.thing.miditools.MidiPortConnector;
import un.thing.miditools.MidiTools;

import java.io.IOException;
import java.sql.Timestamp;

import static un.thing.oboetester.MidiTapTester.NoteListener;
...
public class TapToToneActivity extends TestOutputActivityBase {
    ...
    private class MyNoteListener implements NoteListener {
        ...
        public void onNoteOn(final int pitch) {
            runOnUiThread(() -> {
                trigger();
                getMStreamContexts().get(0).getConfigurationView().setStatusText("MIDI pitch = " + pitch);
