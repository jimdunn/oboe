...
import static un.thing.oboetester.IntentBasedTestSupport.configureStreamsFromBundle;
...
import un.thing.audio_device.AudioDeviceInfoConverter;
...
public class TestDataPathsActivity  extends BaseAutoGlitchActivity {
    ...
    void testPresetCombo(int inputPreset,
        ...
        if (NativeEngine.INSTANCE.isMMapSupported()) {
...
//                2, 0, 2, 1);
    ...
    public void startTestUsingBundle() {
        StreamConfiguration requestedInConfig = mAudioInputTester.requestedConfiguration;
        StreamConfiguration requestedOutConfig = mAudioOutTester.requestedConfiguration;
        configureStreamsFromBundle(getMBundleFromIntent(), requestedInConfig, requestedOutConfig);

        boolean shouldUseInputPresets = getMBundleFromIntent().getBoolean(KEY_USE_INPUT_PRESETS,
                VALUE_DEFAULT_USE_INPUT_PRESETS);
        boolean shouldUseInputDevices = getMBundleFromIntent().getBoolean(KEY_USE_INPUT_DEVICES,
                VALUE_DEFAULT_USE_INPUT_DEVICES);
        boolean shouldUseOutputDevices = getMBundleFromIntent().getBoolean(KEY_USE_OUTPUT_DEVICES,
                VALUE_DEFAULT_USE_OUTPUT_DEVICES);
        int singleTestIndex = getMBundleFromIntent().getInt(KEY_SINGLE_TEST_INDEX,
